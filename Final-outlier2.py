...
def pipelineRemoveOutliers(df, window_size=20, residual_variance_threshold=1.5, plot=True):
    ...
    # Detect obstacles
    if gt: ground_truth = np.interp(df['realtimestamp'].astype(np.int64),groundtruth['timestamp'].astype(np.int64),true_dist * 100)
...
def removeOutliers(data, window_size=20, residual_variance_threshold=1.5, plot=True, gt=False):
    """
